...
def learnknn(k: int, x_train: np.array, y_train: np.array):
    ...
    def predict_func(x_test):
        predictions = []
        for x in x_test:
            distances = np.linalg.norm(x_train - x, axis=1) # compute the distances between x and the train samples
            nn_indices = np.argsort(distances)[:k] # get the indices of the k- nearst neighbors
            nn_labels = y_train[nn_indices] # get the labels of those neighbors
            prediction = np.bincount(nn_labels.astype(int)).argmax() # get the label most in use within the neighbors
            predictions.append(prediction)
        return np.vstack(predictions)

    classifier = {'predict': predict_func}
    return classifier
